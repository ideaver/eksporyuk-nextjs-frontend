...
type Props = {
  ...
  isEdit: boolean;
  handleClose: () => void;
  handleSubmit: (value: ICreateQuizData) => void;
};

let modalsRoot: any = "";

const QuizModal = ({ show, handleClose, handleSubmit, isEdit }: Props) => {
  if (typeof window !== "undefined") {
    modalsRoot = document.getElementById("root-modals") || document.body;
  }

  const currentQuizSelector = useSelector(
    (state: RootState) => state.product.editQuiz
  );
  const stepperRef = useRef<HTMLDivElement | null>(null);
  const stepper = useRef<StepperComponent | null>(null);

  const [data, setData] = useState<ICreateQuizData>(currentQuizSelector);
  ...
  const updateData = (fieldsToUpdate: Partial<ICreateQuizData>) => {
    const updatedData = { ...data, ...fieldsToUpdate };
    setData(updatedData);
  };

  const checkQuizBasic = (): boolean => {
    if (!data.quizBasic.quizName || !data.quizBasic.quizType) {
      return false;
    }
    return true;
  };

  const checkQuizSylabus = (): boolean => {
    if (data.quizSylabus.quizs.length > 0) {
      return true;
    }
