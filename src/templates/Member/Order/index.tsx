...
import useMemberOrderViewModel, { TableProps, tableData } from "./Order-view-model";
import { KTTable } from "@/_metronic/helpers/components/KTTable";
import { KTTableHead } from "@/_metronic/helpers/components/KTTableHead";
import { KTTableBody } from "@/_metronic/helpers/components/KTTableBody";

interface MemberOrderPageProps { }

const MemberOrderPage = ({ }: MemberOrderPageProps) => {
    const {
        breadcrumbs,
        selectedFollupValue,
        handleFollupChange,
    } = useMemberOrderViewModel();
    return (
        <>
            <PageTitle breadcrumbs={breadcrumbs}>Order</PageTitle>
            <KTCard>
                <KTCardBody>
                    <div className="mb-10">
                        <Head />
            ...
            >
                ...
                >
                    ...
                    <th className="fw-bold text-muted text-end min-w-200px">
                        TANGGAL PEMBELIAN <i className="bi bi-arrow-up"></i><i className="bi bi-arrow-down"></i>
                    </th>
                    <th className="fw-bold text-muted text-end min-w-200px">
                        TOTAL HARGA
                    </th>
                ...
                {data.map((row, index) => (
                    <KTTableBody key={index}>
                        <td className="fw-bold">{row.idMemberOrder}</td>
                        <td className="fw-bold">
                            <Link
                                className="text-dark text-hover-primary"
                                href={
                                    "order/" + row.idMemberOrder.replace(" ", "") + "/detail-order/"
                                }
                            >
                                {row.namaProduk}
                            </Link>
